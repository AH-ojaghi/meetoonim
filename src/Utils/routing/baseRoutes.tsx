...
function pushElementToTotalRoutes({totalRoutes, route, path}: { totalRoutes: React.ReactElement[]; route: IRoute; path: string; }) {
    if (route.path.middleware || route.path.preLoadingMethod) {
        totalRoutes.push(
            <Route
                ...
                React.createElement(
                    CheckPreloadings,
                    {
                        children: route.path.middleware?.length === 0 || !route.path.middleware
                            ? route.component
        ...
        totalRoutes.push(<Route key={route.path.path} path={path} element={route.component}/>);
    }
}

function getChildRoute(parentPath: string, child: IRoute): React.ReactElement[] {
    const totalRoutes: React.ReactElement[] = [];
    child.children?.forEach(child => {
        const path = getFullPath(parentPath, child.path.path);
        pushElementToTotalRoutes({totalRoutes, route: child, path});
...
const createRoutes = (routes: IRoute[]): React.ReactElement[] => {
    ...
    routes.forEach((route, i) => {
        pushElementToTotalRoutes({totalRoutes, route, path: route.path.path});
        if (route.children) {
